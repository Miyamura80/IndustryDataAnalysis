...
def _(go):
    ...
    pie_fig_actuator.update_layout(
        ...
        annotations=[
            dict(text="Vertical cuts\n(do not sum)", x=0.5, y=0.5, font_size=14, showarrow=False)
        ],
        showlegend=True,
    )
...
def _(go):
    ...
    actuator_nodes_path = next((path for path in candidate_paths if path.exists()), None)
